...
REM [ "x" = "x$RABBITMQ_FEATURE_FLAGS_FILE" ] && RABBITMQ_FEATURE_FLAGS_FILE=${FEATURE_FLAGS_FILE}
if "!RABBITMQ_FEATURE_FLAGS_FILE!"=="" (
    if "!FEATURE_FLAGS_FILE!"=="" (
        set RABBITMQ_FEATURE_FLAGS_FILE=!RABBITMQ_BASE!\feature_flags
    ) else (
...
if not exist "!RABBITMQ_FEATURE_FLAGS_FILE!" (
    for /f "delims=" %%F in ("!RABBITMQ_FEATURE_FLAGS_FILE!") do mkdir %%~dpF 2>NUL
    copy /y NUL "!RABBITMQ_FEATURE_FLAGS_FILE!" >NUL
)
